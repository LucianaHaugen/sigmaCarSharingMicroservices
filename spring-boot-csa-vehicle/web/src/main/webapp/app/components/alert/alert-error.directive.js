(function() {
    ...
    var csaAlertError = {
        template: '<div class="alerts" ng-cloak="">' +
                        ...
                        '</div>' +
                  '</div>',
        controller: csaAlertErrorController
    };

    angular
        .module('csaApp')
        .component('csaAlertError', csaAlertError);
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope, $translate) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('csaApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-csaApp-error');
                var entityKey = httpResponse.headers('X-csaApp-params');
                if (errorHeader) {
                    ...
                    for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
                        ...
                        var fieldName = $translate.instant('csaApp.' + fieldError.objectName + '.' + convertedField);
